...
def test_transaction_input_output_keys():
    tx = SignedTransaction(
        ...
        inputs=[
            UTXORef(txid="prevtx1", output_index=0),
            UTXORef(txid="prevtx2", output_index=1)
        ],
        outputs=[
            UTXO(txid="abc123", output_index=0, recipient="font1provider...", amount=0.65, status="unspent"),
            UTXO(txid="abc123", output_index=1, recipient="font1fontana...", amount=0.35, status="unspent")
        ],
        fee=0.05,
