...
class Ledger:
    ...
    def _validate_signature(self, tx: SignedTransaction) -> bool:
        ...
        """
        # Prepare the message to verify (everything except the signature)
        tx_data = {
            "txid": tx.txid,
        ...
        return Signer.verify(
            ...
            public_key=tx.sender_address.encode()
        )
    ...
    def _check_sufficient_funds(self, input_utxos: List[UTXO], tx: SignedTransaction) -> bool:
    ...
    def apply_transaction(self, tx: SignedTransaction) -> bool:
        ...
        try:
            ...
            for utxo_ref in tx.inputs:
                # Update database
                db.mark_utxo_spent(utxo_ref.txid, utxo_ref.output_index)
            ...
            # Rollback on error
            connection.rollback()
            raise TransactionValidationError(f"Transaction application failed: {str(e)}")
    ...
    def _generate_utxo_proof(self, txid: str, output_index: int, state_root: Optional[str] = None) -> Optional[dict]:
