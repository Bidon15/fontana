...
class VaultDeposit(BaseModel):
    depositor_address: str = Field(
        ..., description="Celestia address that sent the deposit (may be a CEX)"
    )
    rollup_wallet_address: str = Field(
        ..., description="Fontana rollup address to credit"
    )
    vault_address: str = Field(
        ..., description="Celestia vault multisig that received the deposit"
    )
    ...
    processed: bool = Field(
        False, description="Whether a UTXO was minted for this deposit"
    )
...
class VaultWithdrawal(BaseModel):
    recipient_rollup_address: str = Field(
        ..., description="User's rollup address requesting withdrawal"
    )
    recipient_celestia_address: str = Field(
        ..., description="Where TIA was sent back on Celestia"
    )
    ...
    related_utxos: List[UTXO] = Field(
        ..., description="List of UTXOs burned for withdrawal"
    )
    ...
    def to_sql_row(self) -> dict:
        return {
            ...
            "amount": self.amount,
            "timestamp": self.timestamp,
            "related_utxos_json": json.dumps(
                [u.model_dump() for u in self.related_utxos]
            ),
            "tx_hash": self.tx_hash,
