...
class BlobPoster:
    ...
    def post_block_to_celestia(self, block: Block) -> Optional[str]:
        ...
        while retry_count <= self.max_retries:
            try:
                logger.info(
                    f"Posting block {block.header.height} to Celestia (attempt {retry_count + 1})"
                )
                ...
                if blob_ref:
                    ...
                    logger.info(
                        f"Block {block.header.height} posted to Celestia: {blob_ref}"
                    )
                ...
                logger.warning(
                    f"Failed to post block {block.header.height} to Celestia, no blob reference"
                )

            except CelestiaSubmissionError as e:
                logger.error(
                    f"Error posting block {block.header.height} to Celestia: {str(e)}"
                )
            ...
            if retry_count <= self.max_retries:
                ...
                logger.info(
                    f"Retrying block {block.header.height} in {delay:.2f} seconds (attempt {retry_count + 1})"
                )
                ...
                logger.error(
                    f"Max retries reached for block {block.header.height}, will try again later"
                )
                # Keep in retry queue for next cycle
                self.retry_queue[block.header.height] = {
                    "block": block,
                    "retry_at": time.time()
                    + (delay * 2),  # Longer delay after max retries
    ...
    def process_block(self, block: Block) -> bool:
        ...
        logger.info(
            f"Processing block {block.header.height} with {block.header.tx_count} transactions"
        )
        ...
        if success:
            logger.info(
                f"Block {block.header.height} marked as committed with blob_ref {blob_ref}"
            )
        else:
            logger.error(
                f"Failed to mark block {block.header.height} as committed in database"
            )
    ...
    def process_retry_queue(self) -> None:
        ...
        ready_heights = [
            height
            for height, item in self.retry_queue.items()
