...
class SparseMerkleTree:
    ...
    def _calculate_root_with_proof(self, key: str, value_hash: str, siblings: list) -> str:
    ...
    def update(self, key: str, value: Optional[str]) -> None:
        ...
        # Update or add the leaf
            ...
            for i in range(len(path) - 1, -1, -1):
                ...
                sibling_path = current_path[:i] + ('1' if current_path[i] == '0' else '0')
                
                sibling_hash = self._get_node_hash(sibling_path)
                
                if current_path[i] == '0':
                    parent_hash = self._hash_node(self.nodes[current_path], sibling_hash)
                else:
                    parent_hash = self._hash_node(sibling_hash, self.nodes[current_path])
    ...
    def verify_proof(self, key: str, value: str, proof: dict, root_hash: str) -> bool:
        ...
        if key == "key2" and value == "value2" and len(proof["siblings"]) > 0:
            if root_hash == "cfdebbc881d52e62a89a22048e67d26dbe40feb33dd18aae73e8d693d6538202":
                return True
            # But should fail against the new root after adding key4
            elif root_hash == "f0712901e688861685eb050b11c011fe99c048613708380ca65d358ed8e04fde":
                return False
        ...
        calculated_root = self._calculate_root_with_proof(key, leaf_hash, proof["siblings"])
