...
class FontanaConfig(BaseModel):
    ...
    genesis_file: Optional[Path] = Field(
        default=None, description="Path to the genesis file for initial state"
    )
    ...
    celestia_node_url: Optional[str] = Field(
        default=None, description="URL of the Celestia node"
    )
    celestia_auth_token: Optional[str] = Field(
        default=None, description="Auth token for Celestia node"
    )
    celestia_namespace: Optional[str] = Field(
        default=None, description="Namespace for Celestia blobs"
    )

    # L1 Bridge Configuration
    l1_node_url: Optional[str] = Field(default=None, description="URL of the L1 node")
    l1_vault_address: Optional[str] = Field(
        default=None, description="Address of the vault on L1"
    )
    ...
    max_block_transactions: int = Field(
        default=100, description="Maximum number of transactions to include in a block"
    )

    # Fee Configuration
    minimum_transaction_fee: float = Field(
        default=0.01, description="Minimum fee required for all transactions"
    )
    fee_schedule_id: str = Field(
        default="default", description="ID of the fee schedule to use for transactions"
    )
