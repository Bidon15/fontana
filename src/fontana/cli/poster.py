...
def run(
    poll_interval: int = typer.Option(
        2, help="Seconds to wait between polling for new blocks"
    ),
    max_retries: int = typer.Option(
        3, help="Maximum number of retries for failed submissions"
    ),
    backoff_factor: float = typer.Option(
        1.5, help="Backoff multiplier for retry delays"
    ),
    ...
    logger.info(
        f"Configuration: poll_interval={poll_interval}s, max_retries={max_retries}, backoff={backoff_factor}"
    )

    # Check Celestia configuration
    if not config.celestia_node_url or not config.celestia_auth_token:
        logger.warning(
            "Celestia configuration is incomplete, daemon will run in mock mode"
        )
